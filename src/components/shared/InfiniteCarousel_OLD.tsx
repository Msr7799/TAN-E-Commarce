  ...
  return (
    ...
    >
      ...
      <div className="mb-8 flex items-center justify-center gap-4 px-4">
        ...
        <h2 className="text-center text-sm font-semibold tracking-[0.3em] text-golden uppercase">
          Our Bestsellers
        </h2>
        <span className="h-px max-w-xs flex-1 bg-white/10" aria-hidden="true" />
      </div>
      ...
      >
        {/* Left fade */}
        <div
          className="pointer-events-none absolute top-0 bottom-0 left-0 z-10 w-24 bg-gradient-to-r from-black to-transparent"
          aria-hidden="true"
        />
        {/* Right fade */}
        <div
          className="pointer-events-none absolute top-0 right-0 bottom-0 z-10 w-24 bg-gradient-to-l from-black to-transparent"
          aria-hidden="true"
        />
...
function CarouselCard({ product }: CarouselCardProps) {
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/5] bg-[#f2f0ed]">
        {primaryImage && (
          <Image
      ...
      <div className="flex flex-col gap-1 p-4">
        ...
        <p className="text-xs text-white/50 capitalize">{product.category.replace(/-/g, " ")}</p>
        <p className="text-sm font-bold text-golden">{formatPriceSimple(product.price)}</p>
      </div>
    </motion.div>
  );
}

          ...
          >
                ...
                {freeShippingRemaining > 0 ? (
                  <div>
                    <p className="text-xs text-muted-foreground">
                      {t("cartDrawer.shippingFreeAlert").replace("{amount}", formatPriceSimple(freeShippingRemaining))}
                    </p>
                    <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-beige">
                      <motion.div
                        ...
                        initial={{ width: 0 }}
                        animate={{
                          width: `${Math.min((summary.subtotal / FREE_SHIPPING_THRESHOLD) * 100, 100)}%`,
                        }}
              ...
              <AnimatePresence>
                {items.length === 0 ? (
                  ...
                  >
                    ...
                    <div>
                      <p className="font-semibold text-lg">{t("cartDrawer.emptyTitle")}</p>
                        ...
                        >
                          ...
                          <div className="flex flex-1 flex-col gap-2 min-w-0">
                            <Link
                              href={`/products/${item.product.slug}`}
                              onClick={closeCart}
                              className="text-sm font-semibold leading-tight hover:text-golden transition-colors line-clamp-2"
                            >
                              {itemName}
                            </Link>
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                ...
                                className="flex h-7 w-7 items-center justify-center rounded-full border border-beige hover:border-golden hover:text-golden transition-colors"
                              ...
                              <button
                                ...
                                className="ml-auto flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground hover:bg-red-50 hover:text-red-500 transition-colors"
            ...
            {items.length > 0 && (
              <div className="border-t border-beige px-6 py-5">
                {/* Coupon placeholder */}
                <button className="mb-4 flex w-full items-center gap-2 rounded-xl border border-dashed border-beige px-4 py-2.5 text-sm text-muted-foreground hover:border-golden hover:text-golden transition-colors">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatPriceSimple(summary.subtotal)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between border-t border-beige pt-2 text-base">
                    ...
                    <span className="font-bold text-golden">
                      {formatPriceSimple(summary.total)}
                    </span>
                  </div>
                </div>
